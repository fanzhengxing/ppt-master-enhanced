...
def find_theme_for_project(spec_lock: dict) -> str:
    ...
    # Try multiple common spec_lock key names for background
    bg = (colors.get('background') or colors.get('bg') or '#FFFFFF').strip().upper()
    if not bg.startswith('#'):
        bg = '#FFFFFF'

    # Compute relative luminance from hex to detect dark backgrounds
    try:
        r = int(bg[1:3], 16) / 255
        g = int(bg[3:5], 16) / 255
        b = int(bg[5:7], 16) / 255
    ...
    # Corporate blues (moderately dark)
    if any(k in bg for k in ['003', '004', '005', '006', '1A2', '1B2', '0D1']):
