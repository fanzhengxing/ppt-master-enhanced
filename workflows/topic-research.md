...
## Step 2: Gather via web search (+Deep Research 模式)
...
**Deep Research 模式**（当用户说"深入调研""全面研究""深度了解" 或 用户提供的源材料少于1页时自动触发）：

在标准模式基础上增加：
- **多源交叉**：至少搜索3个不同关键词/角度，覆盖概况、背景、影响、最新动态
- **深度抓取**：拉取 5-8 个权威页面全文（含 Wikipedia、最新新闻、行业报告）
- **结构化综合**：输出独立调研文档 `projects/<topic_slug>_research.md`，包含：
  - 主题概述与关键数据
  - 发展历程/背景
  - 核心内容与细分维度
  - 最新进展（时效性信息）
